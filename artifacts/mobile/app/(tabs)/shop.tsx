    ...
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      <View style={[styles.header, { paddingTop: topPad, backgroundColor: theme.card, borderBottomColor: theme.border }]}>
        <View style={styles.titleRow}>
          ...
          <Pressable style={[styles.sortBtn, { backgroundColor: theme.backgroundSecondary }]} onPress={() => setShowSort(!showSort)}>
            <Feather name="sliders" size={14} color={theme.text} />
            <Text style={[styles.sortBtnText, { color: theme.text }]}>Sort</Text>
          </Pressable>
        </View>
        ...
        <View style={styles.genderTabsRow}>
          {GENDER_TABS.map((g) => (
            <Pressable
              key={g}
              style={[styles.genderTab, { borderBottomColor: gender === g ? Colors.gold : "transparent" }]}
              onPress={() => { setGender(g); setSubcategory("All"); }}
            >
        ...
        {showSort && (
          <View style={[styles.sortDropdown, { backgroundColor: theme.card, borderColor: theme.border }]}>
            {SORT_OPTIONS.map((opt) => (
              ...
              >
                <Text style={[styles.sortOptionText, { color: sortBy === opt ? Colors.gold : theme.text, fontFamily: sortBy === opt ? "Inter_600SemiBold" : "Inter_400Regular" }]}>
                  {opt}
                </Text>
      ...
      <View style={[styles.subcatBar, { backgroundColor: theme.card, borderBottomColor: theme.border }]}>
      ...
      >
        ...
        {productsLoading ? (
          ...
          <View style={styles.emptyState}>
            ...
            <Text style={[styles.emptyText, { color: theme.textSecondary }]}>Try adjusting your filters</Text>
          </View>
        ) : (
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  header: { paddingHorizontal: 16, paddingBottom: 0, borderBottomWidth: 1, zIndex: 20 },
  ...
  sortBtnText: { fontFamily: "Inter_500Medium", fontSize: 12 },
  searchBar: {
    flexDirection: "row", alignItems: "center", borderRadius: 10,
    paddingHorizontal: 12, paddingVertical: 9, borderWidth: 1, gap: 8, marginBottom: 10,
  },
  searchInput: { flex: 1, fontSize: 14, fontFamily: "Inter_400Regular", padding: 0 },
  genderTabsRow: { flexDirection: "row" },
  genderTab: { paddingVertical: 8, paddingHorizontal: 12, borderBottomWidth: 2, marginRight: 2 },
  genderTabText: { fontSize: 14 },
  sortDropdown: {
    position: "absolute", top: "100%", right: 0, left: 0,
    borderRadius: 0, borderTopWidth: 0, borderWidth: 1, zIndex: 100,
  ...
  subcatBar: { borderBottomWidth: 1, height: 46 },
  subcatContent: { paddingHorizontal: 14, paddingVertical: 8, gap: 8, alignItems: "center" },
  subcatChip: { paddingVertical: 5, paddingHorizontal: 13, borderRadius: 14, borderWidth: 1 },
  subcatText: { fontSize: 12 },
  resultCount: { fontFamily: "Inter_400Regular", fontSize: 12, marginBottom: 10, paddingHorizontal: 2 },
